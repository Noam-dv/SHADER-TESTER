...
class NShader extends FlxShader
{
	/*
	 * shaders name (used for the list on the main shader tester state)
	 */
	public var name:String = "shader";

	public function getUniformVariables():Array<Array<Dynamic>>
	{
		var a:Array<Array<Dynamic>> = [];

		for (parameter in __paramBool)
		{
			@:privateAccess if (parameter.__isUniform)
				a.push([parameter.name, Bool]);
		}

		for (parameter in __paramFloat)
		{
			@:privateAccess if (parameter.__isUniform)
				a.push([parameter.name, Float]);
		}

		for (parameter in __paramInt)
		{
			@:privateAccess if (parameter.__isUniform)
				a.push([parameter.name, Int]);
		}
	...
	public function new(name:String = "shader", fragmentSource:String = null, vertexSource:String = null, glslVersion:Int = 120):Void
	{
		this.name = name;
		if (fragmentSource != "")
		{
			...
			{
				NLogs.print('Loading default fragment source...');
				glFragmentSource = processFragmentSource(DEFAULT_FRAGMENT_SOURCE);
			}
			else
			{
				NLogs.print('Loading fragment source from argument...');
	...
	public function setFloat(name:String, value:Float):Void
	{
		...
		if (prop == null)
		{
			// NLogs.print('[WARN] Shader float property ${name} not found.');
			return;
		}
		prop.value = [value];
	}
	...
	public function setInt(name:String, value:Int):Void
	{
		...
		if (prop == null)
		{
			NLogs.print('[WARN] Shader int property ${name} not found.');
			return;
		}
		prop.value = [value];
	}
	...
	public function setBool(name:String, value:Bool):Void
	{
		...
		if (prop == null)
		{
			NLogs.print('[WARN] Shader bool property ${name} not found.');
			return;
		}
		prop.value = [value];
	}
	...
	public function setBoolArray(name:String, value:Array<Bool>):Void
	{
		...
		if (prop == null)
		{
			NLogs.print('[WARN] Shader bool[] property ${name} not found.', 'yellow');
			return;
		}
		prop.value = value;
	}
	...
	public function setSampler2D(name:String, value:BitmapData)
	{
		...
		if (prop == null)
		{
			NLogs.print('[WARNING] Shader sampler2D property ${name} not found.');
			return;
		}
		prop.input = value;
	}
