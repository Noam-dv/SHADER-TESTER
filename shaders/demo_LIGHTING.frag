#pragma header
				...
				uniform float iTime;
				uniform float red_float;
				uniform float green_float;
				uniform float blue_float;
				uniform float Exposure; // Directly scale the effect (0 = no effect, 1 = full)
				...
				float Weight = 0.55 / float(NUM_SAMPLES);
				float Decay = 1.006 - MyDecay / float(NUM_SAMPLES);
				...
				{
					vec4 color = vec4(red_float, green_float, blue_float, 0);
